package com.zdv.shop.common.aspect;
...
@Aspect
@Component
public class CompLogAspect {
	// 注入Service用于把日志保存数据库
	@Autowired
	private LogService logService;
	// 本地异常日志记录对象
	private static final Logger logger = LoggerFactory.getLogger(CompLogAspect.class);

	// Service层切点
	@Pointcut("@annotation(com.zdv.shop.common.annotation.ServiceLog)")
	public void serviceAspect() {
	}

	// Controller层切点
	@Pointcut("@annotation(com.zdv.shop.common.annotation.ControllerLog)")
...
//		// 读取session中的用户
//		Oper_user user = (Oper_user) request.getAttribute("loginUser");
...
//		}
	...
	private ILog getLog(JoinPoint joinPoint, Throwable e){
		HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest();
		// 读取session中的用户
		OpUsers oper_user = (OpUsers) request.getAttribute("loginUser");
		String username = "匿名操作";
		ParamData params = new ParamData();
		if (null != oper_user) {
			username = oper_user.getUloginname();
		}else{
			if(params.containsKey("username")){
				username = params.getString("username");
			}
		...
		log.setType(1);
		if (null != oper_user) {
			//log.setCustomerid(oper_user.getCustomerid());
			log.setCompid((String)request.getAttribute("ucompid"));
		}/*else {
			log.setCustomerid("10000001");
		}*/
		log.setUrl(request.getRequestURI());
		log.setMethod((joinPoint.getTarget().getClass().getName() + "." + joinPoint.getSignature().getName() + "()"));
		log.setParams(params.toString());
